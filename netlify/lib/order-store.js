...
export async function getOrder(id) {
  return store().get(`orders/${id}.json`, { type: 'json', consistency: 'strong' });
}
...
export async function listOrders({ includeClosed = false, limit = 20 } = {}) {
  ...
  const orders = (await Promise.all(keys.map((key) => store().get(key, { type: 'json', consistency: 'strong' }))))
    .filter(Boolean)
...
export async function getSession(chatId) {
  return store().get(`sessions/${chatId}.json`, { type: 'json', consistency: 'strong' });
}
